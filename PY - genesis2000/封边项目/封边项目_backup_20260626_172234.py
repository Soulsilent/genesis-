...
class StepEdgeBandingDialog(QMainWindow):
    ...
    def _get_genesis_instance(self):
        """获取缓存的Genesis实例"""
        if self._genesis_cache is None:
            try:
                self._genesis_cache = GenesisPy3.Genesis()
            except Exception:
                return None
        return self._genesis_cache
    
    def _get_top_instance(self):
        """获取缓存的Top实例"""
        if self._top_cache is None:
            try:
                self._top_cache = genClasses.Top()
            except Exception:
                return None
        return self._top_cache
    
    def refresh_info(self):
        """通过 Gateway + GenesisPy3 实时刷新 JOB/STEP/尺寸/拼版信息"""
        try:
            ...
            print(f"刷新JOB/STEP失败: {e}")
            return

        # 通过 GenesisPy3 获取尺寸和拼版信息（使用缓存实例）
        try:
            top = self._get_top_instance()
            if not top or not top.currentJob or top.currentJob not in top.jobs:
                return
            
            job = top.jobs[top.currentJob]
            if not top.currentStep or top.currentStep not in job.steps:
                return
            ...
            g = self._get_genesis_instance()
            if not g:
                return
            
            # 获取拼版数量 - 优化：合并INFO调用
            pnl_text = ""
            has_set = 'set' in job.steps
            
            if 'pnl' in job.steps:
                # 一次调用获取所有需要的数据
                params = 'nx+ny'
                if has_set:
                    params += '+set_nx+set_ny'
                
                g.INFO(units='mm', entity_type='step',
                       entity_path='%s/pnl' % (g.JOB()),
                       data_type='SR', parameters=params)
                
                nx = list(map(int, g.doinfo.get("gSRnx", [0])))
                ny = list(map(int, g.doinfo.get("gSRny", [0])))
                pnl_num = sum(a * b for a, b in zip(nx, ny))
                pnl_text = f"PNL={pnl_num}"
                
                if has_set:
                    g.INFO(units='mm', entity_type='step',
                           entity_path='%s/set' % (g.JOB()),
                           data_type='SR', parameters='nx+ny')
                    snx = list(map(int, g.doinfo.get("gSRnx", [0])))
                    sny = list(map(int, g.doinfo.get("gSRny", [0])))
    ...
    def start_edge_banding(self):
        ...
        for step_name in checked:
            try:
                ...
                error_list.append(f"{step_name}: {str(e)}")
        
        # 显示执行结果
        if error_list:
            QMessageBox.warning(self, "部分失败",
                              f"成功: {success_count}/{len(checked)}\n\n"
                              f"失败:\n" + "\n".join(error_list))
        else:
            QMessageBox.information(self, "完成", f"成功处理 {success_count} 个STEP")
...
def pnl_fb():
    ...
    if 'pnl' in job.steps:
        g.INFO(units='mm', entity_type='step',
               entity_path='%s/pnl' % (g.JOB()),
               data_type='SR', parameters='nx+ny')
        list_nx = list(map(int, g.doinfo.get("gSRnx", [0])))
        list_yx = list(map(int, g.doinfo.get("gSRny", [0])))
        pnl_num = sum(a * b for a, b in zip(list_nx, list_yx))
        
        if has_set:
            g.INFO(units='mm', entity_type='step',
                   entity_path='%s/set' % (g.JOB()),
                   data_type='SR', parameters='nx+ny')
            set_nx = list(map(int, g.doinfo.get("gSRnx", [0])))
            set_yx = list(map(int, g.doinfo.get("gSRny", [0])))
            set_num = sum(a * b for a, b in zip(set_nx, set_yx))
            pnl_num = pnl_num * set_num
