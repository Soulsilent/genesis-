...
from PyQt5.QtWidgets import (QApplication, QDialog, QVBoxLayout, QHBoxLayout,
                             ...
                             QComboBox, QRadioButton, QButtonGroup, QInputDialog, QLineEdit)
...
auto_primary_offset = auto_long_side + 30
auto_secondary_offset = auto_short_side + 30
...
def is_rectangles_intersect(rec1, rec2):
    """
    判断两个矩形是否相交
    :return: True=相交, False=不相交
    """
    rec1_right = rec1.x + rec1.width
    rec1_top = rec1.y + rec1.height
    rec2_right = rec2.x + rec2.width
    rec2_top = rec2.y + rec2.height

    # 4 种不相交的情况
    case1 = rec1_right < rec2.x      # rec1 在 rec2 左边
    case2 = rec1.x > rec2_right      # rec1 在 rec2 右边
    case3 = rec1_top < rec2.y        # rec1 在 rec2 下边
    case4 = rec1.y > rec2_top        # rec1 在 rec2 上边

    return not (case1 or case2 or case3 or case4)


def rect_offset(p3, pos_lst, x_offset, y_offset):
    """
    如果新文字矩形与已有文字矩形相交，则沿指定方向偏移直到不重叠
    :param p3: 当前文字位置 (x, y)
    :param pos_lst: 已占用的文字矩形列表
    :param x_offset: X轴偏移步长（每次2mm）
    :param y_offset: Y轴偏移步长（每次2mm）
    :return: 避让后的安全位置 (x, y)
    """
    intersect = True
    while intersect:
        current_rec = Rectangle(p3[0] - 7.5, p3[1] - 2.75, 15, 5.5)
        intersect = False
        for rec in pos_lst:
            if is_rectangles_intersect(current_rec, rec):
                p3 = (p3[0] + x_offset, p3[1] + y_offset)
                intersect = True
                break
    return p3
...
class SettingsDialog(QDialog):
    ...
    def _on_naming_remove(self):
        """删除最后一行（已由行内✕按钮代替，保留作为备用）"""
        if self._naming_rows:
            self._remove_mapping_row(self._naming_rows[-1])
...
while True:
    ...
    # 3. 通过 ezdxf 打开 DXF 并写入文字标注（由"是否标注层"控制）
    # ==================== [ezdxf 部分] ====================
    if (dxf_exported and os.path.exists(dxf_path)
        ...
        try:
            ...
            _log(f"[FAIL] ezdxf 处理失败: {e}")
            QMessageBox.warning(None, "图纸已输出", "图纸已输出")
